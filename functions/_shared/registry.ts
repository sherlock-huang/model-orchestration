// Model Registry - Manage and execute model calls (Cloudflare Functions compatible)

import { ModelRegistration, ModelCapabilities } from '../_shared/types';
...
// Helper to get environment variable in Cloudflare Functions
export function getEnv(key: string, env?: Record<string, string>): string {
  if (env && env[key]) {
    return env[key];
  }
  // @ts-ignore - Cloudflare provides process.env in Workers
  return process.env[key] || 'mock-key';
}

// Initialize models with environment variables from Cloudflare
export function initializeModels(env?: Record<string, string>): void {
  globalModelRegistry.register({
    name: 'gpt-4',
    provider: 'openai',
    api_key: getEnv('OPENAI_API_KEY', env),
    capabilities: {
      max_tokens: 8192,
  ...
  globalModelRegistry.register({
    ...
    provider: 'anthropic',
    api_key: getEnv('ANTHROPIC_API_KEY', env),
    capabilities: {
      max_tokens: 200000,
  ...
  globalModelRegistry.register({
    name: 'deepseek-chat',
    provider: 'custom',
    api_key: getEnv('DEEPSEEK_API_KEY', env),
    capabilities: {
      max_tokens: 4096,
